...
def irr(cashflows, max_iter=200, tol=1e-7):
    """Return MONTHLY IRR for a sequence of monthly cashflows."""
...
def annualize(monthly_rate: float) -> float:
    """Convert monthly rate to ANNUAL rate (e.g., 0.01 -> ~0.1268)."""
    if monthly_rate <= -0.999:
        return -1.0
...
def monthly_payment(principal, annual_rate, term_years, amort_years=None):
    ...
    n = int((amort_years or term_years) * 12)
    r = annual_rate / 12.0
    if n <= 0:
        return 0.0
    if r == 0:
